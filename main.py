...
async def get_recipe(message: Message):
	*_, num = message['text'].split()
	try:
		...
		recipe_parts = html_page.find_all('div', class_ = 'detailed_step_description_big')
		...
		for rp_index in range(0, len(recipe_parts)): # rp_index - recipe part index
			...
			await bot.send_photo(chat_id = message['from']['id'],
								photo = open(join('cache/', f"{message['from']['id']}_{rp_index}.png"),'rb'),
								caption = str(text))
		...
		for recipe in all_recipes:
			...
			await bot.send_photo(chat_id = message['from']['id'],
								photo = open(join('cache/', f"{message['from']['id']}_{counter}.png"), 'rb'),
								caption = f'''{"-"*45}
